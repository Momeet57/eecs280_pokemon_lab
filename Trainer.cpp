...
const std::string &Trainer::get_name() const { return name; }
...
Pokemon Trainer::choose_pokemon() {
  Pokemon chosen = this->active_roster[0];
...
//          of their pokemon.
void Trainer::reset() { active_roster = pokemon; }

// EFFECTS: Creates a trainer with the given name and the
//          default set of pokemon.
Trainer *Trainer_factory(const std::string &name) { return new Trainer(name); }
...
Trainer *Trainer_factory(std::istream &is) { return new Trainer(is); }
