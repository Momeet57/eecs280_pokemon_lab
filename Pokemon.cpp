...
Pokemon::Pokemon() : name("Pikachu"), level(1), type(ELECTRIC) {}
...
const std::string &Pokemon::get_name() const { return name; }

// EFFECTS: Returns this pokemon's type
PokemonType Pokemon::get_type() const { return type; }
...
bool operator==(const Pokemon &a, const Pokemon &b) {
  return a.get_name() == b.get_name() && a.get_type() == b.get_type() &&
         a.get_level() == b.get_level();
}
bool operator!=(const Pokemon &a, const Pokemon &b) { return !(a == b); }
...
std::ostream &operator<<(std::ostream &os, const Pokemon &pokemon) {
  os << pokemon.get_name() << " " << pokemon.get_level() << " "
     << pokemon.get_type();
  return os;
}
...
bool Pokemon_battle(const Pokemon &a, const Pokemon &b) {
  if (b.get_level() - a.get_level() > 10) {
    return false;
  } else if (a.get_level() - b.get_level() > 10) {
    return true;
  } else if (a.is_effective_against(b.get_type()) == true) {
    return true;
